...
public class ImageGlitcher {
	...
	public int[][] generateLum(BufferedImage in) {
		...
		for (int y = 0; y < in.getHeight(); y++) {
			...
			for (int x = 0; x < in.getWidth(); x++) {
				lume[y][x] = (int) Math.sqrt((.299 * (in.getRGB(x, y) >> 16 & 0xFF)) 
						+ (.587 * (in.getRGB(x, y) >> 8 & 0xFF)) 
						+ (.114 * (in.getRGB(x, y) >> 0 & 0xFF)));
			}
	...
	public int[][] generateLum(int[][] rgb) {
		...
		for (int y = 0; y < rgb.length; y++) {
			...
			for (int x = 0; x < rgb[0].length; x++) {
				lume[y][x] = (int) Math.sqrt((.299 * (rgb[y][x] >> 16 & 0xFF)) 
						+ (.587 * (rgb[y][x] >> 8 & 0xFF)) 
						+ (.114 * (rgb[y][x] >> 0 & 0xFF)));
			}
	...
	/**
	 * replaces each rgb value of in with the corresponding value from rgb, assuming their dimensions match.
	 * @param in
	 * @param rgb
	 */
	...
	/**
	 * generates a white-on-black int array that represents the canny edge detection of the input.
	 * @param in - 2d int array to process
	 * @return edges - 2d int array edge detection
	 */
	public int[][] generateEdgeDetection(int[][] in) {
		
		CannyEdgeDetection edge = new CannyEdgeDetection(in);
		return edge.toArray();
	...
	public void translateX(int[][] a, int distance, int edgeMode) throws IndexOutOfBoundsException {
		...
		case EDGE_PAD: // pad duplicates the leftmost row in the newly vacated space
		...
		case EDGE_REFLECT: // reflect reflects the leftmost columns across the transform line		
			for (int y = 0; y < a.length; y++) {
				for (int x = a[y].length - 1; x >= distance; x--) {
					a[y][x] = orig[y][x - distance];
				}
		...
		case EDGE_WRAP: // wrap moves the cropped portion of the image to the newly vacated columns
